...
| **Any size, but tell me what it is** | The canvas is fitted to the art so it draws 1:1 with no resampling. The current map is 1407 × 768. What matters is that the two layers below share it exactly. |
| **5 lanes, equal height, full width** | The engine is five lanes across nine columns. |
| **If you paint a grid, keep it uniform** | Each lane painted as a checkerboard *is* the grid a player reads, so the columns are fitted to it. Any uniform pitch works — the current map came out at 95px starting x 285, nine squares, and every column now centres to within 0.0px. What breaks it is an irregular or drifting pitch. |
...
| **Standing props go on the kerbs and margins, never in a lane centre** | Units are placed in lane centres. A potted bonsai in the middle of a lane means a duck standing in its branches, and it forced the previous map's grid inward, costing a whole column. On the kerb between two lanes, the same prop costs nothing and is *better* — it sits in front of the lane behind it and behind the lane in front, which is where the depth effect comes from. |
...
| **Two files: ground, and props on transparency** | This is the one that unlocked depth. The base holds only flat ground — lawn, kerbs, gravel, paving, water. Everything that *stands* (shrubs, potted trees, statues, rocks) goes in a second image with alpha on the same canvas. Then a duck can be covered by the shrub in front of it and cover the one behind. Do **not** try to flatten and extract later: a green shrub on green lawn sits 47 units of colour distance from its background, inside the file's own noise, and no threshold separates them. The layers do not need to be the same resolution — a 73% export scaled back up fine — only the same framing. |
